 ...
 ***************************************************************************/
// default
Adafruit_LSM9DS0_RFDUINO::Adafruit_LSM9DS0_RFDUINO( int32_t sensorID ) {
  _i2c = true;
  ...
  _accelSensor = Sensor(this, &Adafruit_LSM9DS0_RFDUINO::readAccel, &Adafruit_LSM9DS0_RFDUINO::getAccelEvent, &Adafruit_LSM9DS0_RFDUINO::getAccelSensor);
  _magSensor   = Sensor(this, &Adafruit_LSM9DS0_RFDUINO::readMag,   &Adafruit_LSM9DS0_RFDUINO::getMagEvent,   &Adafruit_LSM9DS0_RFDUINO::getMagSensor);
  _gyroSensor  = Sensor(this, &Adafruit_LSM9DS0_RFDUINO::readGyro,  &Adafruit_LSM9DS0_RFDUINO::getGyroEvent,  &Adafruit_LSM9DS0_RFDUINO::getGyroSensor);
  _tempSensor  = Sensor(this, &Adafruit_LSM9DS0_RFDUINO::readTemp,  &Adafruit_LSM9DS0_RFDUINO::getTempEvent,  &Adafruit_LSM9DS0_RFDUINO::getTempSensor);
}

Adafruit_LSM9DS0_RFDUINO::Adafruit_LSM9DS0_RFDUINO(int8_t xmcs, int8_t gcs, int32_t sensorID ) {
  _i2c = false;
  // hardware SPI!
  _csg = gcs;
  _csxm = xmcs;
...
Adafruit_LSM9DS0_RFDUINO::Adafruit_LSM9DS0_RFDUINO(int8_t clk, int8_t miso, int8_t mosi, int8_t xmcs, int8_t gcs, int32_t sensorID ) {
  _i2c = false;
  // software SPI!
  _csg = gcs;
  _csxm = xmcs;
...
bool Adafruit_LSM9DS0_RFDUINO::begin()
{
  if (_i2c) {
    Wire.begin();
...
/***************************************************************************
 PUBLIC FUNCTIONS
 ***************************************************************************/
void Adafruit_LSM9DS0_RFDUINO::read()
{
...
void Adafruit_LSM9DS0_RFDUINO::readAccel() {
...
void Adafruit_LSM9DS0_RFDUINO::readMag() {
...
void Adafruit_LSM9DS0_RFDUINO::readGyro() {
  // Read gyro
  byte buffer[6];
  readBuffer(GYROTYPE, 
...
void Adafruit_LSM9DS0_RFDUINO::readTemp() {
  // Read temp sensor
  byte buffer[2];
  readBuffer(XMTYPE, 
...
void Adafruit_LSM9DS0_RFDUINO::setupAccel ( lsm9ds0AccelRange_t range )
{
...
void Adafruit_LSM9DS0_RFDUINO::setupMag ( lsm9ds0MagGain_t gain )
{
...
void Adafruit_LSM9DS0_RFDUINO::setupGyro ( lsm9ds0GyroScale_t scale )
{
...
*/
/**************************************************************************/
void Adafruit_LSM9DS0_RFDUINO::getEvent(sensors_event_t *accelEvent,
...
*/
/**************************************************************************/
void Adafruit_LSM9DS0_RFDUINO::getSensor(sensor_t *accel, sensor_t *mag,
...
/***************************************************************************
 PRIVATE FUNCTIONS
 ***************************************************************************/
void Adafruit_LSM9DS0_RFDUINO::write8(boolean type, byte reg, byte value)
{
  byte address, _cs;
...
byte Adafruit_LSM9DS0_RFDUINO::read8(boolean type, byte reg)
{
  uint8_t value;
...
byte Adafruit_LSM9DS0_RFDUINO::readBuffer(boolean type, byte reg, byte len, uint8_t *buffer)
{
  byte address, _cs;
...
uint8_t Adafruit_LSM9DS0_RFDUINO::spixfer(uint8_t data) {
...
void Adafruit_LSM9DS0_RFDUINO::getAccelEvent(sensors_event_t* event, uint32_t timestamp) {
...
void Adafruit_LSM9DS0_RFDUINO::getMagEvent(sensors_event_t* event, uint32_t timestamp) {
  memset(event, 0, sizeof(sensors_event_t));
  event->version   = sizeof(sensors_event_t);
...
void Adafruit_LSM9DS0_RFDUINO::getGyroEvent(sensors_event_t* event, uint32_t timestamp) {
  memset(event, 0, sizeof(sensors_event_t));
  event->version   = sizeof(sensors_event_t);
  event->sensor_id = _lsm9dso_sensorid_accel;
...
void Adafruit_LSM9DS0_RFDUINO::getTempEvent(sensors_event_t* event, uint32_t timestamp) {
  memset(event, 0, sizeof(sensors_event_t));
  event->version   = sizeof(sensors_event_t);
...
void Adafruit_LSM9DS0_RFDUINO::getAccelSensor(sensor_t* sensor) {
...
void Adafruit_LSM9DS0_RFDUINO::getMagSensor(sensor_t* sensor) {
  memset(sensor, 0, sizeof(sensor_t));
...
void Adafruit_LSM9DS0_RFDUINO::getGyroSensor(sensor_t* sensor) {
  memset(sensor, 0, sizeof(sensor_t));
...
void Adafruit_LSM9DS0_RFDUINO::getTempSensor(sensor_t* sensor) {
  memset(sensor, 0, sizeof(sensor_t));
